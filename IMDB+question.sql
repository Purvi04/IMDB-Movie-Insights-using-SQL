...
SELECT 
    genre, COUNT(movie_id) AS movie_count_by_genre
FROM
    genre
GROUP BY genre
ORDER BY movie_count_by_genre DESC
LIMIT 1;

-- In last year 2019 genre with the highest number of movies produced 
...
-- finding the top three genres
SELECT genre FROM top_genre
WHERE genre_rank < 4
),
-- identifying the top directors from top three genres
top_director AS (
...
FROM names AS n 
    ...
    selected_genre
WHERE g.genre IN (selected_genre.genre) AND avg_rating>8
...
DENSE_RANK() OVER(ORDER BY COUNT(r.movie_id) DESC) AS actor_rank
FROM ratings as r
...
FROM
    thriller_categories
ORDER BY movie_category DESC, avg_rating DESC;
...
-- identifying top 5 movies year wise from top 3 genres
top_five_movies AS (
SELECT g.genre, year, title AS movie_name, worlwide_gross_income AS worldwide_gross_income,
DENSE_RANK() OVER (PARTITION BY YEAR ORDER BY worlwide_gross_income DESC) AS movie_rank
FROM movie AS m 
	INNER JOIN genre AS g ON m.id= g.movie_id
WHERE g.genre IN (SELECT genre FROM selected_genre))
SELECT 
    *
FROM
    top_five_movies
...
WITH actress_ranks AS ( 
SELECT name AS actress_name, SUM(total_votes) AS total_votes, COUNT(g.movie_id) AS movie_count, 
ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actress_avg_rating, 
DENSE_RANK() OVER(ORDER BY COUNT(g.movie_id) DESC) AS actress_rank
FROM genre AS g
INNER JOIN ratings AS r ON g.movie_id = r.movie_id
INNER JOIN role_mapping AS rm ON g.movie_id=rm.movie_id 
INNER JOIN names AS n ON rm.name_id=n.id
...
SELECT name_id AS director_id, name AS director_name, dir.movie_id, duration,
	   avg_rating AS avg_rating, total_votes AS total_votes,
	   date_published, total_votes * avg_rating AS prod,
...
SELECT 
    ...
    ROUND(SUM(prod)/SUM(total_votes),2) as avg_rating,   
    SUM(total_votes) AS total_votes,
